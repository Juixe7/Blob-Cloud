...
type StorageProvider interface {
	...
	// GenerateStagingUploadURL returns a URL for direct PUT upload to an isolated
	// staging prefix (e.g. staging/{session_id}/{index}). Unverified client uploads
	// are quarantined in staging until cryptographic verification.
	GenerateStagingUploadURL(ctx context.Context, stagingKey string, expires time.Duration) (string, error)

	// PromoteObject promotes an object from staging to its final CAS key (e.g.
	// staging/... -> blocks/<hash>). In S3, this performs an in-region Server-Side
	// Copy (CopyObject) followed by source deletion (zero egress bandwidth).
	// In local storage, this performs an atomic filesystem move.
	PromoteObject(ctx context.Context, srcKey, destKey string) error
}
