...
func TestFileRepository_DeleteRecursive_SingleFile(t *testing.T) {
	...
	deletedRows, orphanedBlocks, err := files.DeleteRecursive(ctx, f.ID, u.ID)
	if err != nil {
		t.Fatalf("DeleteRecursive: %v", err)
	}
	if deletedRows != 1 {
		t.Fatalf("expected 1 deleted, got %d", deletedRows)
	}
	if len(orphanedBlocks) != 2 {
		t.Fatalf("expected 2 orphaned blocks, got %d", len(orphanedBlocks))
	}
...
func TestFileRepository_DeleteRecursive_NestedFolders(t *testing.T) {
	...
	count, _, err := files.DeleteRecursive(ctx, folder.ID, u.ID)
	if err != nil {
		t.Fatalf("DeleteRecursive: %v", err)
	}
	if count != 4 {
...
func TestFileRepository_DeleteRecursive_PreservesUnrelatedFiles(t *testing.T) {
	...
	count, _, err := files.DeleteRecursive(ctx, folderA.ID, u.ID)
	if err != nil {
		t.Fatalf("DeleteRecursive: %v", err)
	}
	if count != 2 {
