...
func (s *Server) HandleDownload(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.zipOps.StreamItemsZip(r.Context(), ids, userID, w); err != nil {
...
// HandleGetRecentFiles returns the user's recently viewed files.
func (s *Server) HandleGetRecentFiles(w http.ResponseWriter, r *http.Request) {
	...
	files, err := s.files.GetRecentViews(r.Context(), userID, 50)
	if err != nil {
		s.log.Error("failed to get recent files", "err", err)
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to retrieve recent files"})
		return
	}
...
// HandleRecordView records a view for a specific file.
func (s *Server) HandleRecordView(w http.ResponseWriter, r *http.Request) {
	...
	// Verify the user has access to the file before recording view
	allowed, err := s.perms.CheckUserPermission(r.Context(), fileID, "", []string{domain.RoleOwner, domain.RoleEditor, domain.RoleViewer})
	if err != nil || !allowed {
		// Just fail silently to prevent leakage
		writeJSON(w, http.StatusForbidden, map[string]string{"error": "access denied"})
		return
	}

	err = s.files.RecordView(r.Context(), userID, fileID)
	if err != nil {
		s.log.Error("failed to record view", "err", err)
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to record view"})
		return
	}
