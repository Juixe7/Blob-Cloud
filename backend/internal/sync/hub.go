...
const (
	...
	EventSyncDelta      = "SYNC_DELTA"
	EventAIMetadataReady = "AI_METADATA_READY"
	EventShareInvitation = "SHARE_INVITATION"
)
...
func NewHub(log *slog.Logger) *Hub {
	return &Hub{
		...
		kickSession: make(chan kickSession),
...
func (h *Hub) Run() {
	for {
		...
		case msg := <-h.notify:
			h.handleNotify(msg)
		case msg := <-h.kickSession:
			h.handleKickSession(msg.sessionID)
		case <-h.done:
			return
		}
	}
...
func (h *Hub) handleKickSession(sessionID string) {
	h.mu.Lock()
	defer h.mu.Unlock()
	
	// We must find the client with this session ID and force logout.
	// This is O(N) over all clients, but it's very rare and fast.
	payload, _ := json.Marshal(NotificationEvent{Type: EventForceLogout})
	var toKick []*Client
	for _, conns := range h.clie {
		for c := range conns {
	...
	for _, c := range toKick {
		...
		// Write pump will send it and we can just let standard disconnect handle unregister,
		// or we close here? It's better to just send payload.
		// We'll let the frontend close the connection after receiving FORCE_LOGOUT.
	}
}
...
// KickSession sends a FORCE_LOGOUT event to the specific session.
func (h *Hub) KickSession(sessionID string) {
	h.kickSession <- kickSession{sessionID: sessionID}
}
